    ...
    def findDuplicate(self, paths):
        ...
        for directory in paths:
            ...
            for f in files:
                tmp2 = f.split('(')
                fname = tmp2[0]
                content = tmp2[1].replace(')', '')

                if content not in cnt2file_dic:
                    cnt2file_dic[content] = [path+'/'+fname]
                else:
                    cnt2file_dic[content].append(path+'/'+fname)
        ret_ls = []

        for k, v in cnt2file_dic.iteritems():
            if len(v) > 1:
                ret_ls.append(v)
