...
class Solution(object):
    def accountsMerge(self, accounts):
        ...
        for item in accounts:
            ...
            usern = userNode(username)
            for email in emails:
                if email not in email2n_dic:
                    emailn = emailNode(email)
                    email2n_dic[email] = emailn
                else:
                    emailn = email2n_dic[email]
                usern.emails.append(emailn)
                emailn.users.append(usern)
            user_ls.append(usern)

        for usern in user_ls:
            if not usern.isVisited:
                all_emails = sorted(self.getemails(usern))
                res_ls.append([usern.user] + all_emails)
        return res_ls

    def getemails(self, usernode):
        if usernode.isVisited:
            return []
        else:
            usernode.isVisited = True
            ret_ls = []
            for emailnode in usernode.emails:
                tmp = []
                if not emailnode.isVisited:
                    tmp += [emailnode.email]
                    emailnode.isVisited = True
                    for usern in emailnode.users:
                        if not usern.isVisited:
                            tmp += self.getemails(usern)
                ret_ls += tmp
            return ret_ls
