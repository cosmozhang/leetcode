...
class Solution(object):
    def exclusiveTime(self, n, logs):
        ...
        for log in logs:
            ...
            log_id = int(log_ls[0])
            log_start_end = log_ls[1]
            log_time = int(log_ls[2])
            if len(stack) > 0 and stack[-1][0] == log_id and stack[-1][1] == 'start' and log_start_end == 'end':
                p = stack.pop(-1)
                ptime = p[2]
                ret_ls[log_id] += log_time - ptime + 1

                if len(stack)>0:
                    old_log = stack[-1]
                    old_log_id, old_start_end = old_log[0], old_log[1]
                    if old_start_end == 'start':
                        ret_ls[old_log_id] -= log_time - ptime + 1
            else:
                stack.append((log_id, log_start_end, log_time))
