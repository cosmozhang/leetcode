...
class SnakeGame(object):
    ...
    def move(self, direction):
        ...
        """
        if direction == 'U':
            newr, newc = self.snake[-1][0]-1, self.snake[-1][1]
        elif direction == 'L':
            newr, newc = self.snake[-1][0], self.snake[-1][1]-1
        elif direction == 'R':
            newr, newc = self.snake[-1][0], self.snake[-1][1]+1
        else:
            newr, newc = self.snake[-1][0]+1, self.snake[-1][1]

        if newr < 0 or newr > self.h-1 or newc < 0 or newc > self.w-1:
            return -1
        
        for b in self.snake[1:]:
            if newr == b[0] and newc == b[1]:
                return -1

        if len(self.food) > 0 and self.food[0] == [newr, newc]:
            self.food.pop(0)
            self.snake.append([newr, newc])
            self.score += 1
            return self.score
        else:
            self.snake.pop(0)
